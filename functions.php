...
add_action('gform_user_registered', 'auto_login_user', 10, 4);
add_action( 'gform_user_registered', 'save_additional_user_meta', 10, 5 );
...
//        var_dump($email);
        ...
        $confirmation    = '<h1>' . 'Email: ' . $email . '</h1>';
        $confirmation    .= '<h2>' . 'Company: ' . $company . '</h2>';
    }
    return $confirmation;
}

add_shortcode('user_info', 'custom_confirmation');

function auto_login_user($user_id, $feed, $entry, $user_pass) {
...
function save_additional_user_meta ($user_id, $feed, $entry) {
    $company = rgar( $entry, '5' );
    update_field('field_64899dc81012f', $company, 'user_'.$user_id);
}
...
function custom_validation($validation_result) {
    ...
    // Define the field IDs for the password and password confirmation fields
    $password_field_id = 6;
    $password_confirmation_field_id = 7;
    $email_field_id = 3;


    // Get the field values from the form submission
    $password_value = rgpost("input_{$password_field_id}");
    $password_confirmation_value = rgpost("input_{$password_confirmation_field_id}");
    $email_value = rgpost("input_{$email_field_id}");

    // Compare the password and password confirmation values
    if ($password_value !== $password_confirmation_value) {
        $validation_result['is_valid'] = false;

        // Get the password confirmation field
        $password_confirmation_field = GFFormsModel::get_field($form, $password_confirmation_field_id);
        if (!empty($password_confirmation_field)) {
            // Set a custom validation message for the password confirmation field
            $password_confirmation_field->failed_validation = true;
            $password_confirmation_field->validation_message = 'Passwords do not match.';
        }
    }
    if (email_exists($email_value)) {
        ...
        // Get the email field
        $email_field = GFFormsModel::get_field($form, $email_field_id);
        if (!empty($email_field)) {
            // Set a custom validation message for the email field
            $email_field->failed_validation = true;
            $email_field->validation_message = 'Email is already registered.';
        }
    }
